...
or:
  ...
  curl http://dfw.rhn.rackspace.com/pub/rhn-org-trusted-ssl-cert-1.0-1.noarch.rpm > rhn-org-trusted-ssl-cert-1.0-1.noarch.rpm ; yum -y --nogpgcheck localinstall rhn-org-trusted-ssl-cert-1.0-1.noarch.rpm
...
yum -y --nogpgcheck install rs-tools rs-release ; rpm --import /etc/pki/rpm-gpg/RACKSPACE-GPG-KEY ; rpm --import /etc/pki/rpm-gpg/IUS-RHN-GPG-KEY
...
"
  ...
  clear
  echo "################################################################################"
  echo "VMware tools should be installed automatically as long as you correctly set up the repo above."
  echo "If that is not an option for whatever reason, the following can be run as long as we have working repositories:"
  echo "
# yum install -y pyxf86config open-vm-tools-devel
  "
  echo "make sure VMware tools not already installed/up-to-date"
  echo "################################################################################"
...
rhel_registration() {
  echo "################################################################################"
  echo "RHEL should already be registered"
  echo "################################################################################"
}
...
rhel_specific_pkgs() {
  ...
  echo ""
  yum install --quiet -y open-vm-tools-devel authconfig krb5-workstation ntp openldap-clients samba-common samba-common-tools sssd sssd-tools glibc.i686 rs-tools rs-release vim screen wget
  echo "################################################################################"
}

vmware_tools() {
  echo "################################################################################"
  echo "VM-ware tools installation should be handled by previous step"
  echo "################################################################################"
}
